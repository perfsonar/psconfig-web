...
if (ver.featureCompatibilityVersion.version == 4.2) {
    // Possibly wait until any indexing operations are finished?
    let indexingOps = db.adminCommand({
        currentOp: true,
        $or: [
    ...
    if (indexingOps.inprog.length != 0) {
        print("I'm in the middle of indexing something. Come back later.");
        // need to exit here
    } else {
        db.adminCommand("listDatabases").databases.forEach(function (d) {
            let mdb = db.getSiblingDB(d.name);
            mdb.getCollectionInfos({ type: "collection" }).forEach(function (
                c
            ) {
                let currentCollection = mdb.getCollection(c.name);
                currentCollection.getIndexes().forEach(function (idx) {
                    ...
                    if (key[0].includes("$**")) {
                        print(
                            "Dropping index: " + idx.name + " from " + idx.ns
                        );
