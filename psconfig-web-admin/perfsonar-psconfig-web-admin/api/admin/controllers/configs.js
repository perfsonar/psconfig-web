...
router.put(
    ...
    function (req, res, next) {
        ...
        importer.import(
            req.body.url,
            req.user.sub,
            function (err, tests, new_config_params) {
                if (err) return next(err);
                res.json({
                    msg: "Created testspecs / host / hostgroups records",
                    tests: tests,
...
router.put(
    ...
    function (req, res, next) {
        db.Config.findById(req.params.id, function (err, config) {
            ...
            if (!config)
                return next(
                    new Error("can't find a config with id:" + req.params.id)
                );
...
router.delete(
    ...
    function (req, res, next) {
        db.Config.findById(req.params.id, function (err, config) {
            ...
            if (!config)
                return next(
                    new Error("can't find the config with id:" + req.params.id)
                );
            if (canedit(req.user, config)) {
