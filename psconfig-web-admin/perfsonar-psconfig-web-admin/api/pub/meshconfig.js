...
function meshconfig_testspec_to_psconfig(testspec, name, psc_tests, schedules) {
    ...
    if (test.type == "rtt") {
        // TODO: figure out a better way to support different field names for different test types
        if ("packet-interval" in spec) {
            spec["packet-interval"] = seconds_to_iso8601(
                spec["packet-interval"]
            );
        }
...
exports.get_config = function (configName, options, next, configObj) {
    ...
    dbTest.init(function (err) {
        ...
        //config = configObj;
        dbTest.Config.findOne({ url: configName })
            .lean()
            .exec(function (err, config) {
                ...
                if (!config) {
                    logger.warn(
                        "404 error: Couldn't find config with name:" +
                            configName
                    );
                    dbTest.disconnect();
                    return next(err);
...
function generate_group_members(
    test,
    group,
    test_service_types,
    type,
    ...
    resolve_hostgroup(group, test_service_types, function (err, hosts) {
        ...
        if (
            "testspec" in test &&
            "specs" in test.testspec &&
            "tool" in test.testspec.specs
        ) {
            set_test_meta(
                test,
                "_tool",
                convert_tool(test.testspec.specs.tool)
            );
        }
        ...
        hosts.forEach(function (host) {
            ...
            if (host.hostname) {
                ...
                if (
                    !host_groups_details[test.name][addr].find(
                        (o) => o.name == host_addr
                    )
                ) {
...
exports._process_published_config = function (_config, opts, cb) {
    ...
    var test_service_types = Object.keys(service_type_obj).map(
        (e) => service_type_obj[e]
    );
    ...
    async.eachSeries(
        ...
        function (err) {
            ...
            if (_config._host_version)
                mc.description += " (v" + _config._host_version + ")";
            ...
            for (var id in host_catalog) {
                ...
                if (!(_host.hostname in psc_hosts))
                    psc_hosts[_host.hostname] = {};
                ...
                if ("ma_urls" in _host && _host.ma_urls.length > 0) {
                    for (var i in _host.ma_urls) {
                        ...
                        var maName =
                            "host-additional-archive" + last_host_ma_number;
                ...
                test_service_types.forEach(function (service) {
                    ...
                    if (opts.ma_override)
                        service.ma = { hostname: opts.ma_override };
                    ...
                    if (format == "psconfig") {
                        ...
                        if (_host.local_ma || _config.force_endpoint_mas) {
                            host.measurement_archives.push(
                                generate_mainfo(service, format)
                            );
                        }
                    }

                    // Handle NEW host main MA (REUSABLE STYLE)
                    if (!("archives" in psc_hosts[_host.hostname]))
                        psc_hosts[_host.hostname].archives = [];
                        ...
                        _host["additional_archives"].forEach(function (_id) {
                            ...
                            if (alreadyExists) {
                                logger.debug(
                                    "_id",
                                    _id,
                                    "already in psc_archives; not adding again"
                                );
                                if (!(_id in host_archive_ids_included)) {
                                    psc_hosts[_host.hostname].archives.push(
                                        psc_archive_ids_included[_id]
                                    );
                                }
                                host_archive_ids_included[_id] = true;
                            } else {
                                new_arch = pub_shared.format_archive(
                                    new_arch[archid],
                                    archid
                                );
                    ...
                    for (var key in extra_mas) {
                        ...
                        var maInfo = generate_mainfo_url(
                            url,
                            format,
                            service.type
                        );
                        ...
                        var maType = maHash[url];
                        if (
                            psc_hosts[_host.hostname].archives.indexOf(key) ==
                            -1
                        ) {
                        ...
                        if (
                            Object.keys(_host._archive) > 0 &&
                            config_service_types.indexOf(service.type) != -1 &&
                            _host._archive.indexOf(maNameHost) == -1
                        ) {
                ...
                if (
                    "archives" in psc_hosts[_host.hostname] &&
                    psc_hosts[_host.hostname].archives.length == 0
                ) {
                    delete psc_hosts[_host.hostname].archives;
                }

                // There are some cases where the host.info object is not defined which causes an exception when laoding mesh config.
                ...
                if (_host.info["location-code"])
                    site.location["postal_code"] = _host.info["location-code"]; //odd one
                org.sites.push(site);
            }
            ...
            if ("ma_custom_json" in _config) {
                ...
                if (customString) {
                    try {
                        ...
                        psc_archives = _.extend(
                            psc_archives,
                            customArchiveConfig
                        );
                    } catch (e) {
                        logger.error(
                            "Custom JSON archive did not validate",
                            e,
                            customString
                        );
                    }
            ...
            if ("archives" in _config) {
                _config.archives.forEach(function (_id) {
                    if (!(_id in archives_obj)) {
                        ...
                        var alreadyExists =
                            archives_obj[_id].data._url in maHash ||
                            _id in psc_archive_ids_included;
                        if (!alreadyExists) {
                            if (
                                _arch !== undefined &&
                                name !== undefined &&
                                archives_obj[_id] !== undefined
                            ) {
                                newArch[name] = archives_obj[_id];
                                newArch = pub_shared.format_archive(
                                    newArch[name],
                                    name + "-" + _id
                                );
            ...
            _config.tests.forEach(function (test) {
                ...
                psc_tests[name].schedule_type =
                    testspec.schedule_type || test.service_type;
                ...
                if (format == "psconfig") {
                    ...
                    meshconfig_testspec_to_psconfig(
                        testspec,
                        name,
                        psc_tests,
                        psc_schedules
                ...
                if (current_test.type == "latencybg") {
                    ...
                    if (
                        "single-ended" in current_test.spec ||
                        "single-ended-port" in current_test.spec
                    ) {
                        testSchema = 2;
                    }
